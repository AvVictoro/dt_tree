...
const fixture = JSON.parse(await fs.readFile(path.join(root, 'catalog/demo/catalog-demo.json'), 'utf8'));
...
check('every_block_represented', fixture.blocks.every(block => block.demoSeries > 0), Object.fromEntries(fixture.blocks.map(block => [block.alias, block.demoSeries])));
...
check('taxonomy3_complete', fixture.indicators.every(item => item.taxonomy3?.topic?.alias && item.taxonomy3?.theme?.alias && item.taxonomy3?.subtheme?.alias), fixture.indicators.length);
